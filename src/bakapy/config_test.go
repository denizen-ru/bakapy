...
var TEST_CONFIG_LOCAL_FILTERS = []byte(`
storage_dir: /tmp/backups/storage
metadata_dir: /tmp/backups/metadata
listen: 127.0.0.1:9876
args:
...
func TestParseConfig_JobsWithLocalFilters(t *testing.T) {
	cfg, _ := ioutil.TempFile("", "test_config")
	cfg.Write(TEST_CONFIG_LOCAL_FILTERS)
	cfg.Close()
	defer os.Remove(cfg.Name())
	...
	if len(config.Jobs["zzz"].LocalFilters) != 2 {
		t.Fatal("LocalFilters length != 2 | ", len(config.Jobs["zzz"].LocalFilters))
	}

	if _, exist := config.Jobs["zzz"].LocalFilters[0]["gzip"]; !exist {
		t.Fatal("Local filters does not parsed")
	}
	if value := config.Jobs["zzz"].LocalFilters[0]["gzip"].Params["level"]; value != "3" {
		t.Fatal("Local filters does not parse values properly")
	}
}
...
func TestParseConfig_TemplateDirectory(t *testing.T) {
	...
	if config.TempDir != "~/.bakapy" {
		t.Fatalf(`Wrong template directory: "%s"; expected "~/.bakapy"`, config.TempDir)
	}
}
